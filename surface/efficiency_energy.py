...
def get_eff_energy_binned(shower_df, zenith_bin):
    ...
    shower_df['zenith_bin'] = shower_df['zenith_bin'].astype(int)
  
    shower_df_stripped = shower_df[shower_df['zenith_bin'] == zenithbin].reset_index(drop=True)

    for i in range(energy_bin_list.shape[0]):
        energy_low = 10 ** log_energy_low_list[i]
        energy_high = 10 ** log_energy_high_list[i]

        energy_df = shower_df_stripped[((shower_df_stripped['energy'] >= energy_low) & (shower_df_stripped['energy'] <= energy_high))]
        trigger_df = energy_df[(energy_df['trigger'] == 1)]
