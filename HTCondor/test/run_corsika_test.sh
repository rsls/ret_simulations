...
output_dir="/pnfs/iihe/radar/corsika/qgsjet/events/${1}/${2}/${3}/coreas/${4}/"
steering_dir="/pnfs/iihe/radar/corsika/qgsjet/events/${1}/${2}/${3}/coreas/${4}/steering/"
log_dir="/pnfs/iihe/radar/corsika/qgsjet/events/${1}/${2}/${3}/coreas/${4}/logs/"
geant_dir="/pnfs/iihe/radar/corsika/qgsjet/events/${1}/${2}/${3}/coreas/${4}/geant/"
temp_dir="${TMPDIR}/${6}/"
...
python /user/rstanley/CORSIKA/corsika/write_inp.py -r ${5} -s ${6} -u ${7} -a ${9} -z ${10} -t ${8} -d ${temp_dir} --season ${11} --time ${12} --location ${13} > ${input_file}
python /user/rstanley/CORSIKA/corsika/write_list.py -a ${9} -z ${10} --location ${13}  > ${list_file}
python /user/rstanley/CORSIKA/corsika/write_reas.py -a ${9} -z ${10} -u ${7} --season ${11} --time ${12} --location ${13} > ${reas_file}
...
cd ${TMPDIR}/${6}/
cp -r * /user/rstanley/CORSIKA/HTCondor/output/


#rm ${TMPDIR}/${6}/DAT${5}.txt

#mv ${input_file} ${steering_dir}
#mv ${list_file} ${steering_dir}
#mv ${reas_file} ${steering_dir}
#mv ${output_file} ${log_dir}
#mv ${geant_file} ${geant_dir}
#mv * ${output_dir}
